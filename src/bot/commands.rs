...
///
/// Fetches and displays the current MLB standings. Uses the in-memory cache
/// for near-instant responses when data is fresh.
#[poise::command(slash_command, description_localized("en-US", "Show MLB standings"))]
pub async fn standings(
    ctx: Context<'_>,
    #[description = "Season year (e.g., 2025). Defaults to current season."] season: Option<u32>,
) -> Result<(), Error> {
    ...
    if season.is_some() && season != ctx.data().config.mlb_season {
        ctx.say(
