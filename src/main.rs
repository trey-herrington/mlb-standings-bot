...
async fn main() -> Result<()> {
    ...
    info!("Starting MLB Standings Bot");
    ...
    let cache = Arc::new(StandingsCache::new(api_client, config.mlb_season));
    ...
    let _warmup_handle = tokio::spawn(async move {
        ...
        match warmup_cache.refresh().await {
            Ok(standings) => {
                info!(
                    "Cache pre-warmed: {} divisions, {} wild card races",
                    standings.divisions.len(),
                    standings.wild_cards.len()
                );
            }
            Err(e) => {
