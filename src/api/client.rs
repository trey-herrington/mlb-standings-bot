...
const BASE_URL: &str = "https://api.balldontlie.io/mlb/v1";
...
/// HTTP client for the balldontlie MLB API.
///
...
impl BallDontLieClient {
    ...
    /// Fetch all 30 MLB teams, retrying on 429 with exponential backoff.
...
fn season_date_ranges(season: u32) -> Vec<(String, String)> {
    let start = NaiveDate::from_ymd_opt(season as i32, 3, 1).expect("valid season start date");
    let end = NaiveDate::from_ymd_opt(season as i32, 10, 31).expect("valid season end date");
...
mod tests {
    ...
    #[test]
    fn season_date_ranges_starts_march_first() {
        let ranges = season_date_ranges(2025);
        assert_eq!(ranges[0].0, "2025-03-01");
    }

    #[test]
    fn season_date_ranges_ends_october_thirty_first() {
        let ranges = season_date_ranges(2025);
        let last = &ranges[ranges.len() - 1];
        assert_eq!(last.1, "2025-10-31");
    }

    #[test]
    fn season_date_ranges_same_year() {
        // MLB season is within a single calendar year, unlike NBA
        let ranges = season_date_ranges(2025);
        for (start, end) in &ranges {
            assert!(start.starts_with("2025"), "Start {start} should be in 2025");
            assert!(end.starts_with("2025"), "End {end} should be in 2025");
        }
    }
    ...
    fn season_date_ranges_different_seasons() {
        for season in [2020, 2023, 2025, 2030] {
            let ranges = season_date_ranges(season);
            assert_eq!(ranges.len(), RATE_LIMIT_BATCH_SIZE);
            assert_eq!(ranges[0].0, format!("{season}-03-01"));
            assert_eq!(ranges.last().unwrap().1, format!("{season}-10-31"));
        }
    }
    ...
    fn season_date_ranges_covers_full_regular_season_window() {
        ...
        // Mar 1 to Oct 31 = 244 days
        assert_eq!(total_days, 244);
    }
    ...
    fn backoff_is_exponential_not_linear() {
        ...
        let backoff_1 = window_secs * 2u64.pow(0);
        let backoff_2 = window_secs * 2u64.pow(1);
        let backoff_3 = window_secs * 2u64.pow(2);
